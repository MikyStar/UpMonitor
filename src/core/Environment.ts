...
dotenv.config({
  path:
    ...
    path.resolve(process.cwd(), 'environments/.env.local'),
});
...
class Env {
  ...
  constructor() {
    ...
    if (this.NODE_ENV === 'test') {
      const specificTestVariables = [
        'VALIDATED_TEST_ACCOUNT',
        'NOT_VALIDATED_TEST_ACCOUNT',
        'NEW_TEST_ACCOUNT',
      ];
      ...
      specificTestVariables.forEach((variable) => {
        if (process.env[variable] === undefined)
          throw new EnvVarMissingError(variable);
        else this[variable] = JSON.parse(process.env[variable]);
      });
    }
  ...
  public isFeatureDisabled = (feat: TogglableFeatures) =>
    this.DISABLED_FEATURES.includes(feat);
}
