...
export class AsyncUtils
{
	...
	static retry = async <T>(
		...
		waitBeforeRetry: number = DateUtils.computeTime( Environment.RETRY_WAIT, 'seconds' )
	) : Promise<T> =>
	{
		return new Promise( async ( resolve, reject ) => // TODO add a timeout feature to wait a bit before testing
		{
			let lastError;
			...
			for( let tryNb = 1; tryNb < ( maxRetries + 1 ); tryNb++ )
			{
				...
				catch( error )
				{
					...
					if( tryNb < maxRetries )
						await SystemUtils.wait( waitBeforeRetry )
				}
			}
